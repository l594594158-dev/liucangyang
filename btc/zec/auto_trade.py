#!/usr/bin/env python3
"""
ZEC v2.0 EMA5/10 1h方向锚定策略 — Binance合约
========================================================
标的:     ZEC/USDT:USDT
交易所:   Binance (合约 fapi)
周期:     1h扫描, 每秒轮询
方向:     1h EMA5 vs EMA10 纯方向锚定 (lv-1闭K)
入场:     五条件AND → 市价单
仓位:     双向各2仓
TP/SL:    +2.5% / -3.0%
杠杆:     25x 逐仓
...
PAUSE_FILE = f'{BASE_DIR}/databases/zec_pause.flag'
NOTIFY_QUEUE = f'{BASE_DIR}/databases/notify_queue.json'
WORK_LOG = f'{BASE_DIR}/logs/zec_work_log.txt'
...
def check_signal(kl_1h, kl_4h):
    """
    五条件AND (lv-1闭K)
    ① EMA方向  ② ADX1h>20  ③ ADX4h<45  ④ 量比>1.5  ⑤ RSI(Long>40/Short<60)
    """
    n = len(kl_1h)
    if n < 50: return None
    ...
    # ② ADX1h > 20 (闭K)
    cond_adx1h = adx1[pi] > ADX_1H_MIN

    # ③ ADX4h < 45 (searchsorted: t_1h - 14400000ms)
    t_signal = kl_1h[pi]['t']
    ...
    if long_dir and cond_adx1h and cond_adx4h and cond_vol and cond_rsi_long:
        return 'long'
    if short_dir and cond_adx1h and cond_adx4h and cond_vol and cond_rsi_short:
        return 'short'
    return None
...
def close_position(side, pos, price, reason):
    try:
        ...
        msg = f"ZEC {side}平仓{reason}: entry={entry:.4f} exit={price:.4f} PnL={pnl*100:+.2f}%"
        log(msg)
        work_log(reason, msg)
        notify(msg)
        return True
...
def open_position(side, price, state):
    try:
        ...
        state['lastentrykl_time'] = int(time.time() // 3600) * 3600 * 1000
        save_state(state)

        msg = f"ZEC {side}开仓: entry={fill_price:.4f} qty={QTY}"
        log(msg)
        work_log('开仓', msg)
        notify(msg)
        return True
...
def main():
    log("="*50)
    log("ZEC v2.0 EMA5/10 1h策略启动")
    log(f"QTY={QTY} | LEV={LEVERAGE}x | TP={TP_PCT*100}% | SL={SL_PCT*100}%")
    log(f"EMA5/10方向 + ADX1h>{ADX_1H_MIN} + ADX4h<{ADX_4H_MAX}")
    log(f"量比>{VOL_RATIO_MIN}x | RSI>{RSI_LONG_MIN}/<{RSI_SHORT_MAX}")
    log(f"仓位: {MAX_POS}仓/边")
    log("="*50)
    notify("ZEC v2.0 EMA5/10策略已启动")
    ...
    while True:
        ...
        try:
            ...
            if len(kl_1h) < 50 or len(kl_4h) < 50:
                time.sleep(10)
                continue
