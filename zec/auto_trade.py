#!/usr/bin/env python3
"""
ZEC合约 趋势回调策略 v1.0（同步BTC v4.3）
- 6条件简化 + TP2.0%/SL1.5% + 双向各1仓
    ...
    try:
        ...
        for p in positions:
            ...
            if side == direction and existing_qty >= QTY:
                log(f"🛡 仓位安全锁 | 已有{direction}仓{existing_qty}ZEC≥{QTY}ZEC | 跳过信号")
                return False
...
def do_close(direction, price, pos_data, reason):
    try:
        ...
        # 清理挂单
        try:
            algos = trade_binance.fapiprivate_get_openalgoorders({'symbol': 'ZECUSDT'})
            for o in algos:
                if o.get('algoStatus') == 'NEW' and o.get('positionSide') == direction:
                    trade_binance.fapiPrivateDeleteAlgoOrder({'symbol': 'ZECUSDT', 'algoId': int(o['algoId'])})
        except:
            pass
    ...
    for p in positions:
        ...
        if side == 'long':
            ...
            if state.get('long_pos') is None:
                ...
                log(f"📌 恢复LONG仓 | {qty}ZEC @ ${exchange_entry:.1f}")
            ...
            if state.get('short_pos') is None:
                ...
                log(f"📌 恢复SHORT仓 | {qty}ZEC @ ${exchange_entry:.1f}")
            elif exchange_entry > 0:
...
def print_status(data, state):
    ...
    print(f"\n╔══ ZEC v1.0趋势回调 {now} ═══")
    print(f"║ 💰 {price:>10,.0f} | RSI:{rsi:.1f} | SMA20:{r5['sma20']:.4f}")
...
# ========== 主循环 ==========
def main():
    log(f"🚀 ZEC v1.0 趋势回调 启动 | {LEVERAGE}x逐仓 | {QTY}ZEC/仓")
